...
- **Agent Container Manager** - Manage multiple isolated runtimes with lifecycle, queueing, snapshots, and metrics
...
### AgentContainerManager (multi-runtime orchestration)

Use `AgentContainerManager` when you want to run many isolated runtimes for agent workloads.
...
const container = await manager.spawn({
  cwd: '/project',
  files: {
    '/project/index.js': 'module.exports = 2 + 2;',
  },
});

const result = await manager.runFile(container.id, '/project/index.js');
console.log(result.exports); // 4

// Clone from snapshot
const clone = await manager.clone(container.id);

// Cleanup
await manager.terminate(container.id);
await manager.terminate(clone.id);
await manager.dispose();
```

Key APIs:
- `spawn(options)`
- `execute(id, code, filename?, options?)`
- `runFile(id, filename, options?)`
- `install(id, packageSpec, installOptions?, operationOptions?)`
- `snapshot(id)` / `clone(id, options?)`
- `list()` / `get(id)` / `terminate(id)` / `terminateAll()` / `dispose()`
- `getMetrics()`
